/*
Copyright 2014-2016 Intel Corporation

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package mac.coreservices.struct;


import org.moe.natj.c.StructObject;
import org.moe.natj.c.ann.Structure;
import org.moe.natj.c.ann.StructureField;
import org.moe.natj.general.NatJ;
import org.moe.natj.general.Pointer;
import org.moe.natj.general.ann.Generated;
import org.moe.natj.general.ann.UncertainArgument;
import org.moe.natj.general.ann.UncertainReturn;
import org.moe.natj.general.ptr.VoidPtr;

@Generated
@Structure(alignment = 2)
public final class LSLaunchFSRefSpec extends StructObject {
	static {
		NatJ.register();
	}
	private static long __natjCache;

	@Generated
	public LSLaunchFSRefSpec() {
		super(LSLaunchFSRefSpec.class);
	}

	@Generated
	protected LSLaunchFSRefSpec(Pointer peer) {
		super(peer);
	}

	@Generated
	@StructureField(order = 0, isGetter = true)
	@UncertainReturn("Options: reference, array Fallback: reference")
	public native FSRef appRef();

	@Generated
	@StructureField(order = 0, isGetter = false)
	public native void setAppRef(
			@UncertainArgument("Options: reference, array Fallback: reference") FSRef value);

	@Generated
	@StructureField(order = 1, isGetter = true)
	public native long numDocs();

	@Generated
	@StructureField(order = 1, isGetter = false)
	public native void setNumDocs(long value);

	@Generated
	@StructureField(order = 2, isGetter = true)
	@UncertainReturn("Options: reference, array Fallback: reference")
	public native FSRef itemRefs();

	@Generated
	@StructureField(order = 2, isGetter = false)
	public native void setItemRefs(
			@UncertainArgument("Options: reference, array Fallback: reference") FSRef value);

	@Generated
	@StructureField(order = 3, isGetter = true)
	@UncertainReturn("Options: reference, array Fallback: reference")
	public native AEDesc passThruParams();

	@Generated
	@StructureField(order = 3, isGetter = false)
	public native void setPassThruParams(
			@UncertainArgument("Options: reference, array Fallback: reference") AEDesc value);

	@Generated
	@StructureField(order = 4, isGetter = true)
	public native int launchFlags();

	@Generated
	@StructureField(order = 4, isGetter = false)
	public native void setLaunchFlags(int value);

	@Generated
	@StructureField(order = 5, isGetter = true)
	public native VoidPtr asyncRefCon();

	@Generated
	@StructureField(order = 5, isGetter = false)
	public native void setAsyncRefCon(VoidPtr value);
}
